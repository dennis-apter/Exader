...
namespace Exader.IO
{
    public partial class FilePath
    {
        ...
        /// </returns>
        public static string CanonicalizePath(string path, out string driveOrHost, out string rootFolder,
            bool strict = false)
        {
                ...
                if (0 <= schemaIndex)
                {
                    ...
                    if (strict)
                    {
                        ...
                        if (volSepIndex < path.Length &&
                            path[volSepIndex] == VolumeSeparatorChar &&
                    ...
                    else
                    {
                        while (volIndex < path.Length &&
                               path[volIndex] != VolumeSeparatorChar &&
                               path[volIndex] != '|')
                            volIndex++;
                    }
                    ...
                    do
                    {
                        ...
                        else
                        {
                            while (volIndexEnd < path.Length &&
                                   path[volIndexEnd] != VolumeSeparatorChar &&
                                   path[volIndexEnd] != '|')
                                volIndexEnd++;
                        }
        ...
        public class Builder : IHierarchy<Builder>, IEquatable<Builder>
        {
            private const int MaxNameLength = 255;
            private const int MinNameLength = 8;

            [NotNull] private readonly string _name = string.Empty;

            [NotNull] private readonly string _value = string.Empty;

            [CanBeNull] private Dictionary<string, string> _annotations;

            [CanBeNull] private HashSet<Builder> _children;
            ...
            public Builder(string value, Builder parent = null)
            {
                if (!string.IsNullOrWhiteSpace(value))
                {
                    ...
                    Extension = SanitizeFileName(ext, '_').Collapse('_', 1);
            ...
            protected Builder(string name, string extension, int index)
            {
                ...
                _limit = _name.Length + Extension.Length;
                Index = index;
            }
            ...
            public bool HasChildren => !_children.IsNullOrEmpty();
            ...
            public string NameWithoutExtension => GetNameWithoutExtension();

            public string NameWithoutExtensionAndIndex => GetNameWithoutExtensionAndIndex();
            ...
            public IEnumerable<Builder> Children => _children ?? Enumerable.Empty<Builder>();
            ...
            public void Add(Builder child)
            {
                ...
                {
                    throw new InvalidOperationException("Parent of path already specified");
                }
                if (child._limit == 0)
                {
                    throw new InvalidOperationException("Can't add root directory as child");
                }

                if (_children == null)
                {
                    _children = new HashSet<Builder>();
                }
            ...
            {
                if (_name == string.Empty && Extension == string.Empty)
                {
                    return RelativeRoot;
                }
                ...
                if (_children == null)
                {
                    if (!TryTrim(maxLength, minLength))
                    {
                        throw new InvalidOperationException(
                            "Неудалось привести имена файлов к компактному представлению.");
                    }
            ...
            {
                var clone = new Builder(_name, Extension, Index);
                CopyTo(clone);
                return clone;
            }
            ...
            private bool TryTrim(int maxLength, int minLength)
            {
                ...
                do
                {
                    ...
                    if (rem < 0)
                    {
                        ...
                        if (minLim < Extension.Length)
                        {
                            minLim = Extension.Length + 1 /*name*/;
                            if (0 < Index)
                            {
