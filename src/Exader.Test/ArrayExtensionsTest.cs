...
{
	/// <summary>
	/// Test for <see cref="ArrayExtensions"/>.
	/// </summary>
	public class ArrayExtensionsTest
	{
		...
		public void Add()
		{
			Assert.Equal(4, new[] { 1, 2, 3 }.Add(4)[3]);
		}
		...
		//[Fact]
		//public void ExpandArgumentOutOfRange()
		//{
		//    var array = new[] { 0, 1, 2, 3, -1, 5, 6, 7, 8, 9 };

		//    Assert.DoesNotThrow(() => array.Expand(-4, 4, -5));
		//    Assert.DoesNotThrow(() => array.Expand(-4, 0, 5));
		//    Assert.DoesNotThrow(() => array.Expand(4, 4, -5));
		//    Assert.DoesNotThrow(() => array.Expand(4, 0, 5));
		//    Assert.DoesNotThrow(() => array.Expand(-4, 5));
		//    Assert.DoesNotThrow(() => array.Expand(4, 5));

		//    Assert.Throws<ArgumentOutOfRangeException>(() => array.Expand(-5, 4, -5));
		//    Assert.Throws<ArgumentOutOfRangeException>(() => array.Expand(-5, 0, 5));
		//    Assert.Throws<ArgumentOutOfRangeException>(() => array.Expand(5, 4, -5));
		//    Assert.Throws<ArgumentOutOfRangeException>(() => array.Expand(5, 0, 5));
		//    Assert.Throws<ArgumentOutOfRangeException>(() => array.Expand(-5, 5));
		//    Assert.Throws<ArgumentOutOfRangeException>(() => array.Expand(5, 5));
		//}
		...
		public void Collapse()
		{
			...
			Assert.Equal(new int?[] { 1, 3, 6 }, new int?[] { 1, 3, 6 }.Collapse());
		}
		...
		{
			var expandedOn1 = new[] { 0, 1, 2, 3, -1, 5, 6, 7, 8, 9 };
			expandedOn1.Expand(-1, 4, -5);

			var expandedOn2 = new[] { 0, 1, 2, 3, -1, 5, 6, 7, 8, 9 };
			expandedOn2.Expand(-2, 4, -5);

			var expandedOn3 = new[] { 0, 1, 2, 3, -1, 5, 6, 7, 8, 9 };
			expandedOn3.Expand(-3, 4, -5);

			// TODO NUnit/xUnit
			Assert.Equal(new[] { 1, 2, 3, -1, -1, 5, 6, 7, 8, 9 }, expandedOn1);
			Assert.Equal(new[] { 2, 3, -1, -1, -1, 5, 6, 7, 8, 9 }, expandedOn2);
			Assert.Equal(new[] { 3, -1, -1, -1, -1, 5, 6, 7, 8, 9 }, expandedOn3);
		}
		...
		{
			var expandedOn1 = new[] { 0, 1, 2, 3, -1, 5, 6, 7, 8, 9 };
			expandedOn1.Expand(1, 4);

			var expandedOn2 = new[] { 0, 1, 2, 3, -1, 5, 6, 7, 8, 9 };
			expandedOn2.Expand(2, 4);

			var expandedOn3 = new[] { 0, 1, 2, 3, -1, 5, 6, 7, 8, 9 };
			expandedOn3.Expand(3, 4);

			// TODO NUnit/xUnit
			Assert.Equal(new[] { 0, 1, 2, 3, -1, -1, 5, 6, 7, 8 }, expandedOn1);
			Assert.Equal(new[] { 0, 1, 2, 3, -1, -1, -1, 5, 6, 7 }, expandedOn2);
			Assert.Equal(new[] { 0, 1, 2, 3, -1, -1, -1, -1, 5, 6 }, expandedOn3);
		}
	}
}